...
export const PickingList = () => {
  ...
  return (
    <div className="absolute inset-0 z-40 bg-white overflow-y-auto custom-scrollbar flex flex-col items-center py-6 px-3 md:py-10 md:px-10">
      
      {/* Cabecera */}
      <div className="w-full max-w-4xl mb-4 md:mb-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-300/80 pb-4 md:pb-6">
        <div>
          <h1 className="text-xl md:text-3xl font-bold text-dicrejart-violet flex items-center gap-3 tracking-wide">
            <ClipboardList className="w-6 h-6 md:w-8 md:h-8 text-dicrejart-violet" />
            Lista de Surtido<span className="hidden sm:inline"> (Picking List)</span>
          </h1>
          <p className="text-sm text-slate-600 font-mono mt-2">ORDEN: ORD-9932-B | ENSAMBLE ESTRUCTURAL</p>
        </div>
        <div className="text-right">
          <p className="text-sm font-mono text-slate-600">PROGRESO DE SURTIDO</p>
          <p className="text-xl md:text-2xl font-bold text-dicrejart-violet">
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 z-10">
                  <h3 className={`text-base md:text-lg font-bold ${isChecked ? 'text-emerald-100 line-through' : 'text-dicrejart-violet'}`}>
                    {item.name}
                  </h3>
                  <p className="text-xs font-mono text-slate-600 mt-1">Material: {item.material}</p>
                  <p className="text-xs font-mono text-slate-600">Dim: {item.dimensions}</p>
                </div>

                {/* Cantidad Gigante */}
                <div className="shrink-0 text-center z-10">
                  <span className="block text-[10px] text-slate-600 tracking-widest uppercase mb-1">Cant</span>
                  <span className={`text-2xl md:text-3xl font-bold font-mono ${isChecked ? 'text-emerald-500/70' : 'text-dicrejart-violet'}`}>
                    x{item.quantity}
                  </span>
                </div>
              </div>
      ...
      <div className={`fixed bottom-10 transition-all duration-500 transform ${isAllChecked ? 'translate-y-0 opacity-100' : 'translate-y-20 opacity-0 pointer-events-none'}`}>
        <button
          ...
          className="flex items-center gap-3 px-6 py-3 md:px-10 md:py-5 bg-emerald-600 hover:bg-emerald-500 text-dicrejart-violet rounded-full font-bold text-base md:text-lg shadow-[0_15px_40px_rgba(16,185,129,0.5)] border border-emerald-400 transition-transform active:scale-95"
        >
