...
export const PickingList = () => {
  ...
  return (
    <div className="absolute inset-0 z-40 bg-industrial-dark overflow-y-auto custom-scrollbar flex flex-col items-center py-10 px-4 md:px-10">
      
      {/* Cabecera */}
      <div className="w-full max-w-4xl mb-8 flex items-center justify-between border-b border-slate-700/80 pb-6">
        <div>
          <h1 className="text-3xl font-bold text-white flex items-center gap-3 tracking-wide">
            <ClipboardList className="w-8 h-8 text-industrial-accent" />
            Lista de Surtido (Picking List)
          </h1>
      ...
      <div className="w-full max-w-4xl grid grid-cols-1 md:grid-cols-2 gap-4 mb-32">
          ...
          consolidatedList.map((item) => {
            ...
            return (
              <div 
                ...
                className={`cursor-pointer transition-all duration-300 rounded-2xl border p-5 shadow-lg relative overflow-hidden flex items-center gap-4 ${
                  isChecked 
              ...
              >
                ...
                <div className="flex-1 z-10">
                  <h3 className={`text-lg font-bold ${isChecked ? 'text-emerald-100 line-through' : 'text-white'}`}>
                    {item.name}
                  </h3>
                ...
                <div className="shrink-0 text-center z-10">
                  ...
                  <span className={`text-3xl font-bold font-mono ${isChecked ? 'text-emerald-500/70' : 'text-industrial-accent'}`}>
                    x{item.quantity}
                  </span>
                </div>
              </div>
      ...
      <div className={`fixed bottom-10 transition-all duration-500 transform ${isAllChecked ? 'translate-y-0 opacity-100' : 'translate-y-20 opacity-0 pointer-events-none'}`}>
        <button
          ...
          className="flex items-center gap-3 px-10 py-5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-full font-bold text-lg shadow-[0_15px_40px_rgba(16,185,129,0.5)] border border-emerald-400 transition-transform active:scale-95"
        >
